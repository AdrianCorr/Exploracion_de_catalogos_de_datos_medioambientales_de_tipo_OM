...
export function renderResults(data) {
  ...
  const total = data.reduce(
    (acum, item) => acum + (Array.isArray(item.processDescription) ? item.processDescription.length : 0), 0);
  ...
  data.forEach(({ processId, processDescription }) => {
    ...
    processDescription.forEach((desc) => {
      ...
      if (processType === "wrf_meteogalicia.modelo_wrf" || processType === "roms_meteogalicia.modelo_roms") {
        ...
        tdName.textContent = desc.nombre || "—";
        rowMain.appendChild(tdName);

        // Intervalo válido (solo start)
        const tdInterval = document.createElement("td");
        tdInterval.textContent = formatDateWithSeconds(desc.validTimeStart || "");
        rowMain.appendChild(tdInterval);

        // Evento click: toggle detalle
        rowMain.addEventListener("click", () => {
          rowDetail.classList.toggle("hidden");
          rowMain.classList.toggle("open");
        });
        ...
        name.innerHTML = "<strong>Nombre:</strong> " + (desc.nombre || "—");
        detailContainer.appendChild(name);
        const p = document.createElement("p");
        p.innerHTML = "<strong>Descripción:</strong> " + (desc.descripcion || "Sin descripción");
